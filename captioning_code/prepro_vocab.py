...
def main(params):
    ...
    for i in videos:
        if i['video_id'] not in video_caption.keys():
            video_caption[i['video_id']] = {'captions': [], 'video_name': []}
        video_caption[i['video_id']]['captions'].append(i['caption'])
        video_caption[i['video_id']]['video_name'].append(i['video_name'])
    # create the vocab
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', type=str, default='caption_data/video_annotation.json',
                        help='msr_vtt videoinfo json')
    parser.add_argument('--info_json', default='caption_data/info.json',
                        help='info about iw2word and word2ix')
    parser.add_argument('--caption_json', default='caption_data/caption.json', help='caption json file')


    parser.add_argument('--word_count_threshold', default=0, type=int,
                        help='only words that occur more than this number of times will be put in vocab') # 1
