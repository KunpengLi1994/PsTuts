...
class VideoDataset(Dataset):
    ...
    def __init__(self, opt, mode):
        ...
        self.with_pop_visual = opt['with_pop_visual']
        self.pop_visual_feats_dir = opt['pop_visual_feats_dir']
        self.with_selected_tool = opt['with_selected_tool']
        self.selected_tool_feats_dir = opt['selected_tool_feats_dir']
        self.without_visual = opt['without_visual']
    ...
    def __getitem__(self, ix):
        ...
        fc_feat = []
        #print(self.captions['video%i'%(ix)])
        #print(self.captions['video%i'%(ix)]['video_name'][0])
        #print()

        for dir in self.feats_dir:
            fc_feat.append(np.load(os.path.join(dir, self.captions['video%i'%(ix)]['video_name'][0] + '.npy')))
        ...
        if self.with_pop_visual == 1:
            pop_visual_feat = np.load(os.path.join(self.pop_visual_feats_dir, self.captions['video%i'%(ix)]['video_name'][0] + '.npy'))
            fc_feat = np.concatenate((fc_feat, pop_visual_feat), axis=1)
            #dim = previous+2048


        if self.with_selected_tool == 1:
            selected_tool_feat = np.load(os.path.join(self.selected_tool_feats_dir, self.captions['video%i'%(ix)]['video_name'][0] + '.npy'))
            fc_feat = np.concatenate((fc_feat, selected_tool_feat), axis=1)
            #dim = previous+39

        if self.without_visual == 1:
            pop_visual_feat = np.load(os.path.join(self.pop_visual_feats_dir, self.captions['video%i'%(ix)]['video_name'][0] + '.npy'))
            selected_tool_feat = np.load(os.path.join(self.selected_tool_feats_dir, self.captions['video%i'%(ix)]['video_name'][0] + '.npy'))
            fc_feat = np.concatenate((pop_visual_feat, selected_tool_feat), axis=1)
